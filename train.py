...
def main():
    ...
    data_test = load_coco_data(data_path='./data', split='test')

    model = CaptionGenerator(word_to_idx, dim_feature=[196, 512], dim_embed=512,
                                       dim_hidden=1024, n_time_step=16, prev2out=True, 
                                                 ctx2out=True, alpha_c=1.0, selector=True, dropout=True)

    solver = CaptioningSolver(model, data, val_data, n_epochs=2, batch_size=128, update_rule='adam',
                                          learning_rate=0.001, print_every=100, save_every=1, image_path='./image/',
                                    pretrained_model='model/lstm/model-20', model_path='model/lstm/', test_model='model/lstm/model-10',
